...
create table if not exists public.weekly_snapshots (
  ...
  construction_maxed boolean not null default false,
  construction_2_maxed boolean not null default false,
  research_maxed boolean not null default false,
  troop_entries jsonb not null default '[]'::jsonb,
...
-- Análise por IA sob demanda no campo "dúvida do dia" — ver
-- docs/BACKLOG-v1.md (nona rodada) e supabase/functions/analyze-checkin.
