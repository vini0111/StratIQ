...
## Adendo 2026-07-20 — vigésima quarta rodada: consolidação de 2 semanas de dados reais + 2 novas cards

Usuário pediu uma análise do que dava para concluir com ~2 semanas de check-ins acumulados (13 check-ins, 2026-07-03 a 2026-07-20) e como consolidar isso na base de conhecimento. Consultado o histórico completo via Supabase; síntese registrada em `docs/KNOWLEDGE-001-Game-Mechanics.md` (nova seção "Observações da conta real"), não só aqui — o objetivo é que rodadas futuras usem esse conhecimento sem precisar reconsultar o banco.

**Achados principais:** ritmo de Fornalha desacelera nos níveis mais altos (21→22 levou 8 dias contra 2-3 dias dos saltos anteriores); VIP ficou parado no nível 4 as duas semanas inteiras acumulando XP lentamente (comportamento normal, já documentado); Poder mais que dobrou (5,38M→12,62M) com salto desproporcional coincidindo com desbloqueio de tier de tropa VIII; ranking de aliança melhorou de 6 para 5; e o achado mais acionável — **gemas e os 4 aceleradores rastreados só cresceram em todos os 13 check-ins, sem nenhum gasto detectado** (gemas 69k→144k; Geral 7,8→24,5 dias; Treino 3,1→8,7; Construção 5,3→10; Pesquisa 3,8→8,2).

Usuário aprovou implementar 3 melhorias a partir disso:

**1) `ACCELERATOR_HOARDING_GENERAL`** (nova card, `accelerator_hoarding_general.json`): mesmo padrão das cards de acúmulo por categoria (limiar de 20 dias), cobrindo o acelerador Geral — que não tinha nenhuma card, apesar de ser o maior estoque observado na conta real (24,5 dias).

**2) `GEMS_APPROACHING_LUCKY_WHEEL_MAX`** (nova card, `gems_approaching_lucky_wheel_max.json`): avisa quando gemas ≥ 120.000 (75% do valor de giro completo) e < 159.000, independente de a Roda da Sorte estar ativa — complementa `SPEND_GEMS_LUCKY_WHEEL`, que só dispara com o evento ativo e o valor já atingido. No ritmo observado (~2.800-4.400 gemas/dia), a conta real cruzaria esse limiar poucos dias depois do check-in mais recente.

**3) Verificação de integridade dos dados de Pets/Equipamento/Prédios.** Usuário reportou que esses domínios são atualizados a cada check-in, contradizendo a leitura inicial (contagem de itens estável desde 2026-07-09/10 sugerindo dados "congelados"). ✅ Verificado via consulta ao conteúdo completo (não só `jsonb_array_length`) dos snapshots: **os dados estão sendo salvos corretamente** — pets sobem de nível, equipamento evolui em nível/raridade, prédios sobem de nível, tudo mudando normalmente check-in a check-in. A leitura anterior estava errada: contagem de itens estável só significa que o jogador está atualizando linhas já existentes, não adicionando linhas novas — não significa dados parados. Nota corrigida em KNOWLEDGE-001. Não havia bug nenhum; erro foi de análise, não de persistência.

## Adendo 2026-07-20 — vigésima quinta rodada: Chief Gear (novo domínio) + widget "Próximos marcos"

Usuário perguntou se dava para avançar em direção a um guia/orientação futura baseada no status real da conta. Cruzando as cards de marco de Fornalha existentes (item A, quarta rodada) com o nível atual da conta real (Fornalha 22, atingido nesta janela), identificado que `FURNACE_22_CHIEF_GEAR` estava disparando — Chief Gear é um domínio concretamente relevante agora, mas sem nenhuma captura de dados (mesma situação de Hero Gear antes da décima nona rodada). Usuário aprovou avançar em duas frentes.

**1) Chief Gear modelado como novo domínio**, mesmo padrão dos domínios da expansão "visão completa da conta" (rodadas 18-21): captura de dados, sem Strategy Card nova. Pesquisado (whiteoutsurvival.wiki, ldshop.gg, bluestacks, heaven-guardian.com, whiteoutdata.com): 6 slots que desbloqueiam juntos na Fornalha 22 — Elmo/Relógio (Lanceiro), Casaco/Calça (Infantaria), Anel/Bengala (Atirador); raridade em cores + sub-tiers (Verde < Azul < Roxo < Roxo T1 < Dourado < Dourado T1 < Dourado T2 < Vermelho < Vermelho T1 < Vermelho T2 < Vermelho T3), e dentro de cada cor, estrelas (0-3). Diferente de Hero Gear, a nomenclatura de slot/raridade **não foi confirmada** contra o cliente em PT — por isso ficou como texto livre + datalist (`KNOWN_CHIEF_GEAR_SLOTS`/`KNOWN_CHIEF_GEAR_TIERS`), mesmo tratamento cauteloso já usado em `KNOWN_BUILDINGS` (vigésima rodada), não como lista fechada.

✅ Implementado: tipo `ChiefGearEntry {slot, tier, stars}` e campo opcional `chiefGearEntries?: ChiefGearEntry[]` em `WeeklySnapshot`. Seção "Chief Gear (opcional)" no `SnapshotForm.tsx`, mesmo padrão de linhas repetíveis (add/remove) de pets/prédios — captura account-wide, não por herói (diferente de Hero Gear). Migração `0014_chief_gear_entries.sql` (coluna `chief_gear_entries jsonb not null default '[]'`, aplicada via MCP do Supabase). Sem Strategy Card nova.

**2) Widget "Próximos marcos"** (`src/lib/roadmap.ts` + `src/components/UpcomingMilestones.tsx`), renderizado no Dashboard logo abaixo do Decision Center. Deliberadamente **não é uma Strategy Card** — não entra no motor de decisão nem no semáforo, é um componente de exibição que lista os próximos desbloqueios de conteúdo por nível de Fornalha (fonte: os mesmos marcos já documentados desde a quarta rodada — 15: Hero Gear, 18: Pets, 20: Mastery Forging, 22: Chief Gear, 25: Chief Charms — mais os saltos de cap de nível de herói), com estimativa de prazo calculada a partir do **ritmo real observado** no histórico da própria conta (`computeFurnacePaceDaysPerLevel`: dias corridos ÷ níveis de Fornalha ganhos entre o primeiro e o último check-in), não uma data fixa. Reaproveita o achado da vigésima quarta rodada (ritmo desacelera em níveis altos) só como aviso textual — o cálculo em si assume ritmo médio constante, é uma estimativa grosseira, não uma previsão do motor.

**Documentação:** nova seção "Chief Gear" em KNOWLEDGE-001-Game-Mechanics.md com as fontes e fatos sourced desta rodada.

## Adendo 2026-07-20 — vigésima sexta rodada: navegação por abas no check-in (inspirada no WoStools)

Usuário fez login no WoStools (wostools.net/account/profile-settings) e gostou da estrutura visual: navegação lateral por domínio (Buildings, Research, Chief Gear, Chief Charms, Hero Gear, Heroes, Pets, etc.), cada aba com seus próprios campos. Pediu para melhorar a interface do StratIQ nessa linha.

Antes de implementar, esclarecido com o usuário (via pergunta direta) dois pontos que não eram óbvios pelo pedido: (1) reestruturar mesmo em abas por domínio, não só polir visualmente o formulário único — confirmado; (2) o modelo de dados continua sendo um check-in novo por vez (snapshot no tempo, histórico preservado), **não** vira "estado editável" solto como no WoStools — confirmado. Isso é uma diferença de fundo importante: o WoStools edita um perfil de conta persistente sem versionamento por data; o StratIQ registra uma série temporal de check-ins, e as Strategy Cards e gráficos de Evolução dependem desse histórico existir. Trocar o modelo de dados quebraria isso, então as abas só reorganizam a **apresentação** de um único check-in — sem sidebar/estado fora do fluxo de check-in.

✅ Implementado: `SnapshotForm.tsx` ganhou navegação por abas (`TABS`, `activeTab` state) com 9 domínios — Visão Geral, Construções, Pesquisa, Tropas, Aceleradores, Heróis, Pets, Chief Gear, Aliança — cada um mostrando só os campos daquele domínio. Duas decisões de design deliberadas, diferentes do WoStools:

- **Barra de abas horizontal com scroll, não sidebar vertical.** O container principal (`#root`) é limitado a 720px (design mobile-first já estabelecido) e o check-in é usado com frequência pelo celular (ver comentário sobre autosave/perda de dados em `SnapshotForm.tsx`, sexta rodada) — uma sidebar vertical não caberia bem em telas estreitas. A barra horizontal com `overflow-x: auto` funciona em qualquer largura.
- **Botão "Salvar atualização" sempre visível, fora das abas**, junto com "Dúvida do dia" — não fica preso à última aba. A maioria dos campos do check-in é opcional (mesmo padrão desde a primeira versão do form), então não faz sentido forçar o jogador a passar por 9 abas antes de conseguir salvar.

Datalists (`buildings-list`, `research-list`, etc.) ficam sempre montadas fora da árvore condicional das abas — o atributo `list` de um `<input>` referencia o `id` em qualquer lugar do documento, não precisa estar na mesma aba do campo que o usa.

CSS novo em `index.css`: `.tab-nav` (flex com scroll horizontal) e `.tab-btn`/`.tab-btn.active` (aba ativa com sublinhado na cor de destaque, mesma paleta já usada em `.rec-item`/`.dot`). Sem mudança de schema/migração — é reorganização de apresentação sobre os mesmos campos e handlers que já existiam.
